      ...
      <div className="relative py-24 border-b border-gray-800">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="max-w-3xl">
            <span className="text-brand-red font-bold uppercase tracking-[0.2em] text-xs mb-4 block">Notre Expertise</span>
            <h1 className="text-5xl md:text-6xl font-serif font-medium mb-6">Domaines d'intervention</h1>
          ...
          {competences.map((comp, idx) => (
            <div key={idx} className="group relative bg-navy-900 border border-gray-800 hover:border-brand-red/50 transition-all duration-500 overflow-hidden">
              <div className="p-10 relative z-10">
                ...
                <h3 className="text-3xl font-serif font-medium mb-4 group-hover:text-white transition-colors">
                  {comp.title}
                </h3>
                <p className="text-gray-400 mb-8 font-light border-l-2 border-gray-700 pl-4 group-hover:border-gold-400 transition-colors">
                  {comp.description}
                </p>
                ...
                <ul className="space-y-3">
                  {comp.details.map((detail, dIdx) => (
                    <li key={dIdx} className="flex items-start text-sm text-gray-300 group-hover:text-gray-100 transition-colors">
                      <div className="h-1.5 w-1.5 bg-brand-red mt-1.5 mr-3 rounded-full opacity-50 group-hover:opacity-100"></div>
                      {detail}
                    </li>
                  ))}
                </ul>
              ...
              <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-white/5 to-transparent rounded-bl-full transform translate-x-16 -translate-y-16 group-hover:scale-150 transition-transform duration-700"></div>
            </div>
          ))}
        </div>

        {/* Call to Action Banner */}
        <div className="mt-20 bg-white text-navy-950 p-12 md:p-16 flex flex-col md:flex-row items-center justify-between gap-8 relative overflow-hidden">
           <div className="relative z-10">
              <h3 className="text-3xl font-serif font-medium mb-2">Un dossier complexe ?</h3>
              <p className="text-gray-600 font-light">Nous analysons votre situation lors d'un premier rendez-vous.</p>
           </div>
           <div className="relative z-10">
              <button className="flex items-center px-8 py-4 bg-navy-950 text-white text-xs font-bold uppercase tracking-widest hover:bg-brand-red transition-colors">
                 Prendre contact <ArrowRight className="ml-3 h-4 w-4" />
              </button>
           </div>
           <div className="absolute top-0 right-0 h-full w-2 bg-brand-red"></div>
        </div>
      </div>
    </div>
  );
